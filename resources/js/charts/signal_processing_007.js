/* ----------------------------*/
/* SIGNAL PROCESSING CHART 007 */
/* ----------------------------*/
(function () {
    let divID = "chart_smoothing3";
    let sectionID = "kernels-for-smoothing-savitzky-golay";
    let myFig = null;
    let axesAlready = false;
    const n = 100;
    let kernel_halfWidth = 1; // Kernel-Halbbreite
    let polynomialOrder = 2; // Ordnung des Polynoms
    ...
    let kernel = mathUtils.savitzkyGolayKernel(kernel_halfWidth, polynomialOrder);
    ...
    Reveal.addEventListener('slidechanged', event => {
        ...
        const spanSlider = document.getElementById("spanSlider3");
        const spanValue = document.getElementById("spanValue3");
        const selector = document.getElementById("polyOrder");
        ...
        spanSlider.addEventListener("input", () => {
            ...
            let newKernel = mathUtils.savitzkyGolayKernel(kernel_halfWidth, polynomialOrder);
    ...
    // evt ist jetzt das Change-Event des Select-Elements
    polynomialOrder = parseInt(evt.target.value, 10);
    // Kernel neu erstellen
    const newKernel = mathUtils.savitzkyGolayKernel(kernel_halfWidth, polynomialOrder);
    // Faltung des Signals mit dem neuen Kernel
    const yData_1_smoothed_new = mathUtils.convolution(yData_1, newKernel);
    const xyData_1_smoothed_new = mathUtils.createXYData(xData, yData_1_smoothed_new);
    // Update der Daten im Chart
