var __codemirror__ = (() => {
  ...
  function skipAtomicRanges(atoms2, pos, bias) {
    for (; ; ) {
      let moved = 0;
      for (let set of atoms2) {
  ...
  function skipAtomsForSelection(atoms2, sel) {
    let ranges = null;
    for (let i = 0; i < sel.ranges.length; i++) {
      let range = sel.ranges[i], updated = null;
      if (range.empty) {
        let pos = skipAtomicRanges(atoms2, range.from, 0);
        if (pos != range.from)
          updated = EditorSelection.cursor(pos, -1);
      } else {
        let from = skipAtomicRanges(atoms2, range.from, -1);
        let to = skipAtomicRanges(atoms2, range.to, 1);
  ...
  var jsonLang = StreamLanguage.define(jsonMode);
  var basicSetup = [
    lineNumbers(),
  ...
  var rLanguageSupport = [rLang, rTheme, syntaxHighlighting(rHighlightStyle)];
  ...
  window.createPythonEditor = function(parent, doc2) {
